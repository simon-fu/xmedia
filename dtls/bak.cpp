...
static SSL_CTX *xdtls_get_ssl_ctx(void) {
  return ssl_ctx;
}
...
gint xdtls_srtp_init_(const gchar *server_pem, const gchar *server_key) {
...
static int mtu = 1472;
static void janus_dtls_bio_filter_set_mtu(int start_mtu) {
  if(start_mtu < 0) {
    dbge("Invalid MTU...\n");
    return;
  }
...
static void janus_dtls_srtp_destroy(xdtls_srtp *dtls) {
}

static xdtls_srtp *xdtls_srtp_create(void *ice_component, janus_dtls_role role) {
...
main (int argc, char **argv)
{
  ...
  dbgi("EVP_MAX_MD_SIZE=%d", EVP_MAX_MD_SIZE);
  dbgi("is_client = %d", is_client);

  if(!is_client){
    ret = xdtls_srtp_init_("certs/server-cert.pem", "certs/server-key.pem");
    if(ret){
      return 1;
    }
  }else{
    ret = xdtls_srtp_init_("certs/client-cert.pem", "certs/client-key.pem");
    if(ret){
      return 1;
    }
  }
