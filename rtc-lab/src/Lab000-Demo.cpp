...
//        int rtp_payload_size = size-header_size-ext_size;
//        
//        const int OPUS_RTP_PT = 103;
//        if (rtp_pt == OPUS_RTP_PT) {
//            int channels = 1;
//            int frame_size = opus_decode(session->audioDecoder_
//                                         , rtp_payload, rtp_payload_size
//                                         , session->pcm_buf, sizeof(session->pcm_buf)/sizeof(session->pcm_buf[0])
//                                         , 0);
...
//        bool gotUDP = false;
//        while(1){
//            int bytesRecv = session->udpsock_->recv(data, dataSize);
...
//                uint8_t buf8[8];
//                be_set_i64(now_ms, buf8);
//                be_set_u32(session->localPort, buf);
//                tlv_file_write2(dump_tlv_, TLV_TYPE_TIME_PORT_UDP, sizeof(now_ms), &now_ms, bytesRecv+4, buf);
//            }
...
//                    }
//                    
////                    for(int i = 0; i < session->peerAudioSSRCs_.groupNum(); ++i){
////                        uint32_t ssrc = session->peerAudioSSRCs_.mediaSSRC(i);
////                        if(ssrc > 0){
////                            be_set_u32(ssrc, &buf[4]);
////                            session->udpsock_->send(buf, 8, session->remoteAddr_);
////                            ++audioRRs;
////                        }
////                    }
////                    for(int i = 0; i < session->peerVideoSSRCs_.groupNum(); ++i){
////                        uint32_t ssrc = session->peerVideoSSRCs_.mediaSSRC(i);
////                        if(ssrc > 0){
////                            be_set_u32(ssrc, &buf[4]);
////                            session->udpsock_->send(buf, 8, session->remoteAddr_);
////                            ++videoRRs;
...
//                uint8_t buf8[8];
//                be_set_i64(now_ms, buf8);
//                tlv_file_write2(dump_tlv_, TLV_TYPE_TIME_JSON, 8, buf8, (int)msg.length(), (void*)msg.c_str());
//            }
//        }
//    }
//    
...
//            
//            // TODO: check create result
//            session->audioDecoder_ = opus_decoder_create(16000, 1, &ret);
//            
//            if(newSession){
...
//                uint8_t buf8[8];
//                be_set_i64(now_ms, buf8);
//                tlv_file_write2(dump_tlv_, TLV_TYPE_TIME_JSON, 8, buf8, (int)json_length, (void*)json);
//                tlv_file_write2(dump_tlv_, TLV_TYPE_TIME_JSON, 8, buf8, rsp_len, (void*)rsp_msg);
//            }
//        }while(0);
//        if(newSession){
//            delete newSession;
...
//
//static
//int feedAudioData(AudioBuffer * audioBuffer){
//    int len = 1024;
//    for(;;){
//        // 计算还剩多少音频数据
...
//        SDL_QueueAudio(audioBuffer->devId, audioBuffer->data+audioBuffer->offset, bytes);
...
//        wanted_spec.callback = NULL; // 回调函数
