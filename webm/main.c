...
int mfile_loop(mfile_t obj, void * context, mfile_loop_func_t cbfunc){
	...
	unsigned char * data_buf = malloc(data_buf_size);
	int frame_no = 0;
...
int main0(int argc, char *argv[]){
	...
	unsigned char * video_buf = malloc(video_buf_size);

	unsigned int audio_buf_size = 1*1024*1024;
	unsigned char * audio_buf = malloc(video_buf_size);
...
// 	, mediacodec_id_t audio_codec_id, int audio_samplerate, int audio_channels);
		...
		unsigned char * video_keyframe = malloc(256*1024);
...
int main1(int argc, char *argv[]){
	...
	unsigned char * buf = malloc(video_buf_size);
...
// 	, mediacodec_id_t audio_codec_id, int audio_samplerate, int audio_channels);
		...
		rr = rr_open(output_file, video_codec_id, video_width, video_height, 15
			, playout_codec_id, playout_samplerate, playout_channels);
		if(!rr){
			break;
		}
